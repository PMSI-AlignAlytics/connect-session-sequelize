...
module.exports = function SequelizeSessionInit(Store) {
	...
	SequelizeStore.prototype.get = function getSession(sid, fn) {
		debug('SELECT "%s"', sid);
		this.sessionModel.find({where: {'sid': sid}}).done(function(err, session) {
			if (err) {
				debug('Error finding session: %s', err);
				fn(err);
			} else {
				if (!session) {
	...
	SequelizeStore.prototype.set = function setSession(sid, data, fn) {
		...
		this.sessionModel.findOrCreate({where: {'sid': sid}, defaults: {'data': stringData}}).spread(function sessionCreated(session) {
			if(session['data'] !== stringData) {
				...
				session.save().done(function updated(err, session) {
					if (err) {
						debug('Error updating session: %s', err);
						if (fn) {
							fn(err);
						}
					} else {
	...
	SequelizeStore.prototype.destroy = function destroySession(sid, fn) {
		...
		this.sessionModel.find({where: {'sid': sid}}).done(function foundSession(err, session) {
			if (err){
				debug('Error finding session: %s', err);
				fn(err);
			} else {
				...
				if (session === null) {
					...
					session.destroy().done(function destroyedSession(err) {
						if (err) {
							debug('Error destroying session: %s', err);
							fn(err);
						} else {
	...
	SequelizeStore.prototype.length = function calcLength(fn) {
		this.sessionModel.count().then(function sessionsCount(err, c) {
			if (err) {
				fn(err);
			} else {
				fn(null, c);
